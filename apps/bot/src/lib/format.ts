export class PlaceHolder<T extends Readonly<Record<string, unknown>>> {
  ...
  constructor(
    private _prefix: PlaceHolder.Prefix = '!',
    brackets: PlaceHolder.Brackets = '[]',
  ) {
    this.holder = new Map();
  ...
  private _parse(str: string, params: Partial<T>): string {
    return str.replace(
      new RegExp(
        `(?<!\\\\)\\${this._prefix}(?<!\\\\)\\${this.startBracket}(\\w+)(?<!\\\\)\\${this.endBracket}`,
        'g',
      ),
      (input, key) => {
  ...
  list() {
    return Array.from(
      this.holder.keys(),
      (key) => `${this._prefix}${this.startBracket}${key}${this.endBracket}`,
    );
  }
  ...
  set prefix(prefix: PlaceHolder.Prefix) {
    if (!PlaceHolder.allowPrefix.includes(prefix))
      throw new TypeError(
        `${prefix} can't be used for prefix. Only the following values can be used (${PlaceHolder.allowPrefix.join(
          ',',
        )})`,
      );
    this._prefix = prefix;
  ...
  set brackets(brackets: PlaceHolder.Brackets) {
    if (!PlaceHolder.allowBrackets.includes(brackets))
      throw new TypeError(
        `${brackets} can't be used for prefix. Only the following values can be used (${PlaceHolder.allowBrackets.join(
          ',',
        )})`,
      );
    const [start, end] = brackets.split('');
    if (!start || !end) {
      throw new TypeError('Invalid brackets value');
    }
...
export namespace Duration {
  ...
  export function toMS(text: string) {
      ...
      .match(
        RegExp(
          `${Object.entries(durations).reduce(
            (p, [short, { long }]) => `${p}((?<${short}>-?(\\d*\\.\\d+|\\d+))(${short}|${long}))?`,
            '',
          )}$`,
          'i',
        ),
    ...
    return Object.entries(match?.groups ?? {}).reduce(
      (p, [key, value = 0]) => p + Number(value) * durations[key as List].time || 0,
      0,
    );
  }
  ...
  export function parse(ms = 0, pass: List[] = []): Partial<Record<List, number>> {
    ...
    return Object.fromEntries(
      Object.entries(durations)
        ...
        .map(([k, v], i, arr) => {
          const prevUnitTime = arr[i - 1]?.[1].time;
          return {
            ...v,
            short: k,
            diff: prevUnitTime ? prevUnitTime / v.time : Number.NaN,
          };
        })
        .map(({ short, long, time, diff }) => ({
          short,
          long,
          duration: Number.isNaN(diff)
            ? Math.floor(absMs / time)
            : Math.floor(Math.floor(absMs / time) % diff),
        }))
        .filter(({ duration }) => duration !== 0)
        .map((v) => [v.short, v.duration]),
    ) as Partial<Record<List, number>>;
  }
  ...
  export function format(ms = 0, template: string | boolean = '', pass: List[] = []): string {
    if (typeof template === 'string' && template)
      return holder.parse(
        template,
        parse(ms, Object.keys(durations).filter((v) => !template.includes(`%{${v}}`)) as List[]),
      );

    return Object.entries(parse(ms, pass))
      .map(([short, duration]) => {
