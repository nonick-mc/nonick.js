...
GlobalFonts.registerFromPath(resolve(process.cwd(), 'src/fonts/OpenSans-Regular.ttf'), 'Captcha');
...
export class Captcha {
  ...
  drawDecoy(option?: drawDecoyOption) {
    ...
    for (const char of decoyText)
      this.ctx.fillText(char, rnd.nextInt(30, this.width - 30), rnd.nextInt(30, this.height - 30));
  }
  ...
  drawLine(option: drawLineOption = {}) {
    ...
    this.ctx.globalAlpha = opt.opacity;
    this.ctx.beginPath();
    if (!this.coord[0]) return;
    this.ctx.moveTo(this.coord[0][0], this.coord[0][1] + rnd.nextInt(-opt.blur, opt.blur));
    for (let i = 1; i < this.coord.length; i++) {
      const coord = this.coord[i];
      if (!coord) continue;
      this.ctx.lineTo(coord[0], coord[1] + rnd.nextInt(-opt.blur, opt.blur));
    }
    this.ctx.stroke();
    this.ctx.closePath();
  }
  ...
  drawText(option: drawTextOption = {}) {
    ...
    for (let i = 0; i < this.coord.length; i++) {
      ...
      if (!coord || !char) continue;
      this.ctx.save();
      this.ctx.translate(...coord);
      if (opt.skew) this.ctx.transform(1, Math.random(), rnd.nextInt(20) / 100, 1, 0, 0);
      if (opt.rotate) this.ctx.rotate((rnd.nextInt(-opt.rotate, opt.rotate) * Math.PI) / 100);
...
function getRandomText(length: number) {
  return new Int8Array(length).reduce((p) => p + String.fromCharCode(rnd.nextInt(65, 90)), '');
}
