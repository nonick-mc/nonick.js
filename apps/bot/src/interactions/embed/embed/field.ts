...
const addField = [
  new Button({ customId: 'nonick-js:embedMaker-addField' }, async (interaction) => {
  ...
  new Modal({ customId: 'nonick-js:embedMaker-addFieldModal' }, (interaction) => {
...
const removeField = [
  new Button({ customId: 'nonick-js:embedMaker-removeField' }, async (interaction) => {
    const embed = interaction.message.embeds[0];
    const components = interaction.message.components;

    if (embed.fields.length === 0) return;
    if (embed.fields.length === 1)
      return reloadEmbedMaker(interaction, EmbedBuilder.from(embed).setFields().data);
      ...
      .then((i) => {
        ...
        if (i.customId === indexSelectCustomId && i.isStringSelectMenu()) {
          ...
          newComponents[1].addComponents(
            ButtonBuilder.from(components[1].components[3] as APIButtonComponent),
          );
