...
const button = new Button({ customId: 'nonick-js:embedMaker-footer' }, (interaction): void => {
...
const modal = new Modal({ customId: 'nonick-js:embedMaker-footerModal' }, (interaction) => {
  ...
  if (!text && iconURL)
    return interaction.reply({
      content: '`❌` アイコンのURLを設定する場合は、テキストも入力する必要があります。',
      ephemeral: true,
    });
