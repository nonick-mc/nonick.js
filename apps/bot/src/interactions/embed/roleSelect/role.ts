...
const addRole = [
  new Button({ customId: 'nonick-js:embedMaker-selectRole-addRole' }, async (interaction) => {
    if (interaction.message.components[0].type !== ComponentType.ActionRow) return;
  ...
  new Modal({ customId: 'nonick-js:embedMaker-selectRole-addRoleModal' }, async (interaction) => {
    if (
...
const removeRole = [
  new Button({ customId: 'nonick-js:embedMaker-selectRole-removeRole' }, async (interaction) => {
    if (interaction.message.components[0].type !== ComponentType.ActionRow) return;
